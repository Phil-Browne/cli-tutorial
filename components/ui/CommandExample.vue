  ...
  <div class="rounded-xl border border-gray-700 overflow-hidden my-4 font-mono text-sm flex flex-col">
    ...
    <div class="flex items-center justify-between gap-3 px-4 py-3 bg-gray-900">
      ...
      >
        ...
        <svg v-else class="w-3.5 h-3.5 text-green-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
    ...
    <p v-if="description" class="px-4 py-2 text-xs text-gray-500 bg-gray-900/50 border-t border-gray-800 font-sans">
      {{ description }}
    </p>

    <!-- Optional output -->
    <div v-if="output" class="border-t border-gray-800 px-4 py-3 bg-gray-950 text-gray-400 whitespace-pre-wrap text-xs leading-relaxed">{{ output }}</div>
  </div>
</template>
