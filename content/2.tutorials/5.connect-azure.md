...
::info-card{type="note" title="Using `--later` instead?"}
`vxc delete --later` keeps the VXC active until the end of the billing cycle. If you defer cancellation, you must also defer the ExpressRoute circuit deletion — leave the circuit in place until the VXC actually cancels, otherwise you'll trigger the inconsistent-state scenario the warning above describes.
