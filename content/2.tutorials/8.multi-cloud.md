...
```bash
# Buy commands output human-readable text, not JSON — look up the UID afterwards
megaport-cli mcr buy --json-file ./mcr-config.json
MCR_UID=$(megaport-cli mcr list --output json | jq -r '.[] | select(.name == "Cloud Router SYD") | .uid')
echo "MCR: $MCR_UID"
```
...
`aws-vxc.json` (replace `portUid` with your MCR UID and `productUID` with the AWS partner port UID from `partners list --company-name "Amazon Web Services"`):

```json
{
  "vxcName": "MCR → AWS Direct Connect",
  "rateLimit": 1000,
  "term": 12,
  "portUid": "<MCR-UID>",
  "aEndConfiguration": {
...
AWS_VXC=$(megaport-cli vxc list --output json | jq -r '.[] | select(.name == "MCR → AWS Direct Connect") | .uid')
...
`azure-vxc.json` (Azure auto-discovers the partner port from the service key — no `productUID` needed):

```json
{
  "vxcName": "MCR → Azure ExpressRoute",
  "rateLimit": 1000,
  "term": 12,
  "portUid": "<MCR-UID>",
  "aEndConfiguration": {
...
AZ_VXC=$(megaport-cli vxc list --output json | jq -r '.[] | select(.name == "MCR → Azure ExpressRoute") | .uid')
...
```json
{
  "vxcName": "MCR → Google Cloud Interconnect",
  "rateLimit": 1000,
  "term": 12,
  "portUid": "<MCR-UID>",
  "aEndConfiguration": {
  ...
  "bEndConfiguration": {
    "productUID": "<GOOGLE-PARTNER-PORT-UID>",
    "partnerConfig": {
      "connectType": "GOOGLE",
...
```bash
megaport-cli vxc buy --json-file ./gcp-vxc.json
GCP_VXC=$(megaport-cli vxc list --output json | jq -r '.[] | select(.name == "MCR → Google Cloud Interconnect") | .uid')
...
jq --arg uid "$MCR_UID" '.portUid = $uid' aws-vxc-template.json > aws-vxc.json
jq --arg uid "$MCR_UID" '.portUid = $uid' azure-vxc-template.json > azure-vxc.json
jq --arg uid "$MCR_UID" '.portUid = $uid' gcp-vxc-template.json > gcp-vxc.json
...
megaport-cli vxc delete "$AWS_VXC" --force
echo "AWS VXC deleted"

megaport-cli vxc delete "$AZ_VXC" --force
echo "Azure VXC deleted"

megaport-cli vxc delete "$GCP_VXC" --force
echo "GCP VXC deleted"

megaport-cli mcr delete "$MCR_UID" --force
echo "MCR deleted"
