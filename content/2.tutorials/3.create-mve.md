...
::info-card{type="warning" title="MVE deletion is immediate and final"}
v0.10 removed the end-of-billing-cycle defer option for MVEs — `delete` is immediate and cannot be undone. Use `--safe-delete` if you want the command to fail when VXCs are still attached.
::
