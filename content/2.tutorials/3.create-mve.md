...
::info-card{type="tip" title="Renaming vNICs later"}
You can rename vNICs after the MVE is live with `mve update --vnics`, passing one object per existing vNIC in order:

```bash
megaport-cli mve update <MVE-UID> --vnics '[{"description":"Data Plane"},{"description":"Management"}]'
```

Only the `description` is mutable — the vNIC count and VLANs are fixed once the MVE is provisioned.
::
