...
::info-card{type="note" title="Deferring with `--later`"}
`vxc delete --later` keeps the VXC active until the end of the billing cycle. If you defer the cancellation, leave the AWS Virtual Interface in place until the VXC actually cancels — deleting the VIF while the VXC is still up creates the orphan-VIF state the warning above describes.
