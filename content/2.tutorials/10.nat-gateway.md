...
NAT Gateway is billed on the speed tier. Start with the lowest combination that covers expected peak concurrent sessions — you can resize later by deleting and recreating.
::
...
`nat-gateway update` changes mutable fields like name and cost centre:

```bash
megaport-cli nat-gateway update <NAT-UID> --name "Sydney NAT — Prod" --cost-centre "NET-001"
```
...
**Plan capacity once**
- Pick a speed/session-count tier that covers peak load with headroom — there's no in-place resize
