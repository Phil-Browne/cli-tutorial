...
| Constraint | Value |
|---|---|
| Contract terms | 1, 12, 24, 36 months (LAG ports: 1, 12, 24 only) |
| Max resource name length | 64 characters (ports, MVE) |
| Output formats | `table`, `json`, `csv`, `xml`, `go-template` |
...
The JSON schema for `vxc buy` nests partner config inside `bEndConfiguration.partnerConfig`:

```json
{
  "vxcName": "AWS via Sydney",
  "rateLimit": 1000,
  "term": 12,
  "portUid": "<A-END-UID>",
  "aEndConfiguration": {
...
The CLI flags (`--a-end-uid`, `--b-end-partner-config`) map to these fields. For Azure VXCs, you can omit `bEndConfiguration.productUID` — the CLI looks up the partner port from the service key.
...
|---|---|---|
| `connectType` | Yes | `"AWS"` (Virtual Interface) or `"AWSHC"` (Hosted Connection) |
| `ownerAccount` | Yes | AWS account ID (12 digits) |
| `type` | Yes (interactive) / Recommended (JSON) | `"private"` (Private VIF) or `"public"` (Public VIF) |
| `asn` | Yes (interactive) / Recommended (JSON) | Customer-side BGP ASN |
| `name` | No | Connection name shown in AWS Console (max 255 chars) |
| `amazonAsn` | No | Amazon-side BGP ASN (default: 64512) |
| `authKey` | No | BGP MD5 authentication key |
| `prefixes` | No | CIDR blocks to advertise (required for Public VIFs) |
| `customerIpAddress` | No | Customer-side BGP peer IP (e.g. `169.254.10.1/30`) |
| `amazonIpAddress` | No | Amazon-side BGP peer IP (e.g. `169.254.10.2/30`) |
...
{
  "connectType": "IBM",
  "account_id": "32hexcharactersexactly0123456789",
  "customer_asn": 64512,
  "name": "MEGAPORT",
  "customer_ip_address": "169.254.0.1/30",
  "provider_ip_address": "169.254.0.2/30"
}
```
...
|---|---|---|
| `connectType` | Yes | `"IBM"` |
| `account_id` | Yes | IBM Cloud account ID — exactly 32 hex characters |
| `customer_asn` | Yes (unless other end is an MCR) | Customer ASN (1–64495, 64999, 131072–4199999999, or 4201000000–4201064511) |
| `name` | No | Connection name (max 100 chars; defaults to `MEGAPORT`) |
| `customer_ip_address` | No | Customer-side IPv4 with mask (default `/30` from `169.254.0.0/16`) |
| `provider_ip_address` | No | Provider-side IPv4, must share subnet with `customer_ip_address` |

> **Note:** IBM partner-config field names use `snake_case` (not camelCase) — `account_id`, `customer_asn`, etc.
...
Managed network address translation service inside the Megaport fabric. **Delete is immediate and irreversible** — there is no soft-delete or restore window, and no `lock`/`unlock` commands.
...
| `sessionCount` | integer | NAT session count |
| `term` | integer | Contract term in months (1, 12, 24, or 36) |
| `autoRenew` | boolean | Whether the contract auto-renews |
| `diversityZone` | string | Diversity zone (optional) |
| `serviceLevelReference` | string | Service-level reference (optional) |
| `costCentre` | string | Cost centre label |
| `resourceTags` | map\<string, string\> | Key-value tags |

### Telemetry types

`BITS`, `PACKETS`, `SPEED` — passed comma-separated to `nat-gateway telemetry --types`. Use either `--days` (1–180) or an explicit `--from`/`--to` range in RFC3339 format.

### Valid speed/session combinations

Run `megaport-cli nat-gateway list-sessions` for the current matrix of supported speed and session-count pairs.
