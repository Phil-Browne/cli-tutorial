...
`--base-url` (v0.10) and `--token-url` (v0.11) let you target a non-default API endpoint without switching `--env`. Useful for staging or dev hosts. The CLI emits a warning if either URL is not HTTPS.
