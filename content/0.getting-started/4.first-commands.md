...
# Verify your credentials and see the active identity, company, and environment
megaport-cli whoami
```
...
# Inter-location latency (median RTT, previous month)
...
::info-card{type="tip" title="Filter JSON with --query"}
Need just one field or a filtered subset? Use the global `--query` flag (JMESPath, requires `--output json`):

```bash
megaport-cli locations list --output json --query "[?metro=='Sydney'].name"
megaport-cli ports list --output json --query "[?provisioningStatus=='LIVE'].uid"
```
::
