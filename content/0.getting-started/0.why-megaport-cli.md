...
The CLI covers the entire Megaport platform: Ports, VXCs, MCRs, MVEs, IX connections, NAT Gateways, Locations, Partners, Service Keys, Users, Managed Accounts, and Billing Markets. If you can do it in the portal, you can do it from the command line.

### Declarative provisioning

The `megaport-cli apply` command takes a YAML or JSON file describing your full topology — ports, MCRs, MVEs, and the VXCs between them — and provisions everything in dependency order. Cross-resource references use Go-template syntax (`{{.port.MyPort}}`) so VXCs can point at resources created earlier in the same file. Pair it with `--dry-run` to validate before committing.
...
Ready to get started? Follow the [Installation](/getting-started/installation) guide to get it running on your machine -- or jump straight to the [Browser Demo](/demos/browser-terminal) to try it with zero setup.
