...
| Priority | Method | How |
|---|---|---|
| 1 | **Per-command flag** | `--profile <name>`, `--env <env>` on a single command |
...
::info-card{type="tip" title="Verify your active identity"}
Run `megaport-cli whoami` (or the equivalent `megaport-cli auth status`) to confirm which user, company, environment, and profile the CLI is using. Add `--output json` for scriptable output.
::
