...
## Common Megaport CLI use cases

::info-card{type="tip" title="Provision and change services quickly"}
Create, validate, update, and decommission Ports, VXCs, MCRs, MVEs, and IX services without switching between multiple portals.

**Typical commands:** `buy`, `validate`, `update`, `delete`, `status`
::

::info-card{type="note" title="Troubleshoot and verify connectivity"}
Confirm service state, inspect topology, and verify endpoints during incidents or change windows.

**Typical commands:** `status`, `topology`, `get`, `list`
::

::info-card{type="info" title="Automate repeatable network operations"}
Use JSON input/output and config profiles to script provisioning workflows and standard checks in CI/CD pipelines.

**Typical commands:** `apply`, `--json`, `--json-file`, `config create-profile`, `config use-profile`
::

::info-card{type="warning" title="Demo and discovery workflows"}
Run a browser-based CLI session for walkthroughs, then move to local install for production operations.

**Start here:** [Browser Demo](/demos/browser-terminal) and [First Commands](/getting-started/first-commands)
::
