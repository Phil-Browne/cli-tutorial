...
description: A visual guide to every resource the Megaport CLI manages — Ports, VXCs, MCRs, MVEs, IX, NAT Gateway, and more
duration: 15 minutes
difficulty: Beginner
---
...
The Megaport CLI manages nine resource types that together form a software-defined network. Understanding how they relate to each other is the foundation for everything else.
...
**Key attributes:**
- Network service type (e.g. `Sydney IX`, `Los Angeles IX`) — the specific exchange
- ASN (your Autonomous System Number)
- MAC address of the IX interface
- Peering VLAN
- Rate limit in Mbps

```bash
# List your IX connections (filter by ASN, location, network service type, etc.)
megaport-cli ix list

# Get details and BGP peer information
megaport-cli ix get <IX-UID>

# Buy an IX connection on an existing port
megaport-cli ix buy \
...
# View peers on an IX
megaport-cli ix get <IX-UID> --output json
```
...
::info-card{type="note" title="NAT Gateway"}
A **NAT Gateway** is a managed network address translation service that runs inside the Megaport fabric. Use it to map private addresses to a shared public-facing pool when connecting branch sites or cloud workloads through Megaport.
::

**Key attributes:**
- Speed in Mbps
- Session count (number of concurrent NAT sessions)
- Contract term (1, 12, 24, or 36 months)
- Diversity zone (optional)
- Auto-renew flag

```bash
# Discover the valid speed / session-count combinations
megaport-cli nat-gateway list-sessions

# List your NAT Gateways
megaport-cli nat-gateway list

# Get details on a specific NAT Gateway
...
# Telemetry for a NAT Gateway (BITS, PACKETS, or SPEED for the last N days)
megaport-cli nat-gateway telemetry <NAT-UID> --types BITS --days 7
```

::info-card{type="warning" title="NAT Gateway delete is immediate"}
`nat-gateway delete` is irreversible — there is no soft-delete or grace period. Use `--force` to confirm. NAT Gateways have no `lock`/`unlock`/`restore` commands.
::
